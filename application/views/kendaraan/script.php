...
<script>
    ...
    function get_user(x) {
        $.ajax({
            type: "post",
            data: 'id=' + x,
            url: '<?php echo base_url('kendaraan/getbyid') ?>',
            dataType: 'json',
            cache: 'false',
            success: function(hasil) {
                $('[name="id"]').val(hasil['id_kendaraan']);
                $('[name="nama_kendaraan"]').val(hasil['nama_kendaraan']);
                $('[name="jml_roda"]').val(hasil['jml_roda']);
            }
        });
    }
